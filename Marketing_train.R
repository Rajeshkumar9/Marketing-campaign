...
mar_tr = read.csv("marketing_tr.csv", header = T, na.strings = c(" ", "", "NA"))
...
marketing_train$responded = as.factor(as.character(marketing_train$responded))
marketing_train$day_of_week = as.factor(marketing_train$day_of_week)
...
for(i in 1:ncol(marketing_train)){
  ...
  if(class(marketing_train[,i]) == 'character'){
...
#    marketing_train[,i]  = as.numeric(as.character(marketing_train[,i]))
  }
}
...
#***( in case the outcome/targe variableis not a factor,convert it like this- train$responded = as.factor(train$responded))
...
#Confusion matrix:
confmatrix= table (test$responded, C50_Predictions)

confusionMatrix(confmatrix)


###Random Forest
RF_model = randomForest(responded ~ ., train, importance = TRUE, ntree = 300)
...
exec[1:3,]
#Make rules more readable (with the variable names)
...
readablerules[1:3,]
 ...
 logit_Predictions = predict(logit_model, newdata = test, type = "response") #we used response typt to get probability values to fix our own cutoff value.
 
 
 #convert prob
 logit_Pred = ifelse(logit_Predictions > 0.5, 1, 0)
 
 
 ##Evaluate the performance of classification model
 ConfMatrix_RF = table(test$responded, logit_Pred)
 
 #accuracy = (TP+TN)/(TP+TN+FP+FN)
 # = (1298+47)/(1298+47+121+16)= 0.9075 
 #False Negative rate
 #FNR = FN/(FN+TP) = 121/(121+47) = 0.7202
 
 #Accuracy: 90.75(with 0.5 cutoff value)
 #FNR: 72.02(with 0.5 cutoff value)
 # with cutoff 0.4 i get accuracy = 90.48 and FNR is highly reduced to 64.8 percent.
 ...
 #Predict test data
 KNN_Predictions = knn(train[, 1:16], test[, 1:16], train$responded, k = 9)
 ...
 #accuracy = 90.14 - for k =9
 #FNR= FN/(FN+TP) = 33/(33+55)= 37.5 - for k =9
 ...
 #type = raw gives probabilities of the classes
 NB_Predictions_prob = predict(NB_model, test[,1:16], type = 'raw')
...
#we can also find accuracy by below:
#statical way
